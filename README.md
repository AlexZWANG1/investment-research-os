...
Augur 不是单个 prompt，而是一组可组合的 agent workflows。Skills 是系统的“思考协议”：它们定义什么时候触发、读什么上下文、如何生成结构化输出、输出应该落到哪一层。

| Skill | 作用 |
|---|---|
| `augur` | 运行一次完整的 source -> signal -> graph reasoning pipeline |
| `wiki-think` | 把原始材料编译成 source / signal / contradiction |
| `research` | 在限定问题中做多轮证据搜索、估值和对抗式判断 |
| `deep-research` | 在基础研究稿之上做启发式追问和反共识挖掘 |
| `hypothesis-thinking` | 把不确定问题转成假说、证据和置信度更新 |
| `portfolio-allocation` | 把判断转成仓位和风险表达 |
| `report-digest` | 将研报、纪要、访谈压缩成可进入 wiki 的结构化信号 |
| `flex-memory` | 记录预测、复盘偏差、校准判断习惯 |
| `product-review` | 用产品和交互视角评估 AI 产品体验 |
| `visualizer` | 把复杂研究结果转成可读的可视化页面 |
...
移动端和 replay prototype 的截图适合放进 README，但需要用原始图片文件提交到 `assets/`。聊天窗口里的图片不会自动进入 git。
...
For implementation details, see [Technical Architecture](docs/ARCHITECTURE.md) and [Roadmap](docs/ROADMAP.md).
